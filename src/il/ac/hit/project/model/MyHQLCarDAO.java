...
public class MyHQLCarDAO implements ICarDAO {
	...
	@Override
	public void addCar(int carId, String model, String year, int price, int branchId,String carImageUrl) throws CarRentException {
		Session session = null;
		Transaction tx = null;
		try {
			session = factory.openSession();
			tx = session.beginTransaction();
			Car car = new Car(carId, model, year, price, branchId,carImageUrl);
	...
	public boolean doesCarNeedsToBeUpdateOrAdded(Car car) throws CarRentException {
		...
		try {
			...
			if (cars.size() > 0) {
				...
				addCar(car.getCarId(), car.getModel(), car.getYear(), car.getPrice(), car.getBranchId(), car.getImageUrl());
			}
		}
